...
const handleMessage = async (
  ...
  try {
    ...
    if (filaescolhida === "2ª Via de Boleto" || filaescolhida === "2 Via de Boleto") {
      ...
      const urlSgp = await Setting.findOne({
        where: {
          key: "ipsgp",
          companyId
        }
      ...
      const app = await Setting.findOne({
        where: {
          key: "appsgp",
          companyId
        }
      ...
      //if (urlmkauth != "" && Client_Id != "" && Client_Secret != "") {
      let urlSgpFinal = urlSgp.value
      let tokenSgp = token.value
      let appSgp = app.value

      if (urlSgpFinal.substr(-1) === '/') {
        urlSgpFinal = urlSgp.value.slice(0, -1);
      }
      if (isNumeric(numberCPFCNPJ) === true) {
        if (cpfcnpj.length > 2) {
          const isCPFCNPJ = validaCpfCnpj(numberCPFCNPJ)
          if (isCPFCNPJ) {
            try {
              await sleep(2000)
              const textMessage = { text: formatBody(`Aguarde! Estamos consultando na base de dados!`, contact) };
              await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, textMessage);
              var data = new FormData();
              data.append('token', tokenSgp);
              data.append('app', appSgp);
              data.append('cpfcnpj', numberCPFCNPJ);

              var config = {
                method: 'post',
                maxBodyLength: Infinity,
                url: `${urlSgpFinal}/api/ura/consultacliente/`,
                headers: {
                  ...data.getHeaders()
                },
                data: data
              ...
              axios(config)
                .then(async function (response) {
                  ...
                  let cliente = response.data.contratos[0].razaoSocial;
                  let contratos = "";

                  response.data.contratos.forEach(option => {
                    contratos += `*${option.contratoId}* - ${option.endereco_logradouro} - ${option.endereco_numero}\n`;
                  });
                  sleep(2000)
                  const bodycliente = { text: formatBody(`Olá *${cliente}*, localizei seu cadastro, estou buscando mais algumas informações. Por favor aguarde.`, contact) };
                  await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, bodycliente);
                  sleep(2000)
                  const bodycontratos = { text: formatBody(`Para qual contrato voce deseja atendimento ?\n\n${contratos}\n\nDigite apenas o *número* do contrato!`, contact) };
                  await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, bodycontratos);
                  let idContrato;
                  await sleep(2000)
                  ...
                  while (check == false) {
                    await sleep(3000)
                    const lastMessageNew = await Ticket.findByPk(ticket.id);
                    const text = lastMessageNew.lastMessage.includes('Digite apenas o *número* do contrato!');
                    if (text == false) {
                      lastMessage = lastMessageNew.lastMessage;
                      check = true
                      break;
                    }
                  }

                  const bodycontratosselecionado = { text: formatBody(`Estou localizando mais informações do contrato Nº *${lastMessage}*`, contact) };
                  await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, bodycontratosselecionado);
                  sleep(2000)
                  var data = new FormData();
                  data.append('token', tokenSgp);
                  data.append('app', appSgp);
                  data.append('contrato', lastMessage);

                  var config_segundavia = {
                    method: 'post',
                    maxBodyLength: Infinity,
                    url: `${urlSgpFinal}/api/ura/fatura2via/`,
                    headers: {
                      ...data.getHeaders()
                    },
                    data: data
                  ...
                  axios(config_segundavia)
                    .then(async function (response_segundavia) {
                      let nome = response_segundavia?.data?.razaoSocial
                      let valorCorrigido = response_segundavia?.data?.links[0]?.valor.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })
                      let linhadigitavel = response_segundavia?.data?.links[0]?.linhadigitavel
                      let codigopix = response_segundavia?.data?.links[0]?.codigopix
                      let vencimento = moment(response_segundavia?.data?.links?.vencimento).format('DD/MM/YYYY')
                      let link = response_segundavia?.data?.link
                      if (response_segundavia?.data?.status === 0) {
                        const bodyError = { text: formatBody(`Não encontrei nenhuma fatura para este contrato! Retorne o contato e fale com atendente`, contact) };
                        await sleep(2000)
                        await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, bodyError);
                        await UpdateTicketService({
                          ticketData: { status: "closed" },
                          ticketId: ticket.id,
                          companyId: ticket.companyId,
                        ...
                        await sleep(2000)
                        await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, bodyBoleto);
                        const bodyLinha = { text: formatBody(`${linhadigitavel}`, contact) };
                        await sleep(2000)
                        await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, bodyLinha);
                        if (codigopix !== null) {
                          const bodyPdf = { text: formatBody(`Este é o *PIX COPIA E COLA*`, contact) };
                          await sleep(2000)
                          await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, bodyPdf);
                          const bodyqrcode = { text: formatBody(`${codigopix}`, contact) };
                          await sleep(2000)
                          await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, bodyqrcode);
                          let linkBoleto = `https://chart.googleapis.com/chart?cht=qr&chs=500x500&chld=L|0&chl=${codigopix}`
                          await sleep(2000)
                          const bodyQrCodePix = { text: formatBody(`Este é o *QR Code* do PIX`, contact) };
                          await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, bodyQrCodePix);
                          await sendMessageImage(wbot, contact, ticket, linkBoleto, "")
                        }
                    ...
                    .catch(async function (error) {
                      console.log(error);
                      const bodyerro = { text: formatBody(`Opss! Ocorreu um erro. Tente novamente.`, contact) };
                      await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, bodyerro);
                ...
                .catch(async function (error) {
                  console.log(error);
                  const bodyerro = { text: formatBody(`Opss! Não localizei seu cadastro! Tente novamente.`, contact) };
                  await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, bodyerro);
